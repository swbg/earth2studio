...
class TestProcessGeocatalogIngestionEdgeCases:
    """Additional geocatalog tests for remaining uncovered branches."""

    def _redis_side_effect(self, request_id, storage_info, pending_metadata):
        storage_info_key = f"inference_request:{request_id}:storage_info"
        metadata_key = f"inference_request:{request_id}:pending_metadata"
        return lambda k: {
            storage_info_key: (
                json.dumps(storage_info) if storage_info is not None else None
            ),
            metadata_key: (
                json.dumps(pending_metadata) if pending_metadata is not None else None
            ),
        }.get(k)

    def test_missing_storage_queue_returns_none_triggers_fail_workflow(self):
        """When storage/metadata missing and queue_next returns None, calls fail_workflow (line 899)."""
        request_id = "foundry_fcn3_workflow:exec_1"
        with (
            patch("earth2studio.serve.server.cpu_worker.config") as mock_config,
            patch("earth2studio.serve.server.cpu_worker.redis_client") as mock_redis,
            patch(
                "earth2studio.serve.server.cpu_worker.queue_next_stage"
            ) as mock_queue,
            patch("earth2studio.serve.server.cpu_worker.fail_workflow") as mock_fail,
            ...
            mock_redis.get.side_effect = self._redis_side_effect(
                request_id, storage_info=None, pending_metadata=None
            )
            mock_queue.return_value = None
            mock_fail.return_value = {"success": False}

            result = process_geocatalog_ingestion(
                workflow_name="foundry_fcn3_workflow", execution_id="exec_1"
            )
    ...
    def test_no_blob_url_queue_returns_none_triggers_fail_workflow(self):
        """When no blob_url and queue_next returns None, calls fail_workflow (line 927)."""
        request_id = "foundry_fcn3_workflow:exec_1"
        with (
            patch("earth2studio.serve.server.cpu_worker.config") as mock_config,
            patch("earth2studio.serve.server.cpu_worker.redis_client") as mock_redis,
            ...
            mock_redis.get.side_effect = self._redis_side_effect(
                request_id,
                storage_info={},  # no blob_url key
                pending_metadata={"parameters": {}},
            )
            mock_queue.return_value = None
            mock_fail.return_value = {"success": False}
    ...
    def test_unsupported_workflow_queue_returns_none_triggers_fail_workflow(self):
        """When workflow unsupported and queue_next returns None, calls fail_workflow (line 952)."""
        request_id = "other_workflow:exec_1"
        with (
            patch("earth2studio.serve.server.cpu_worker.config") as mock_config,
            patch("earth2studio.serve.server.cpu_worker.redis_client") as mock_redis,
            ...
            result = process_geocatalog_ingestion(
                workflow_name="other_workflow", execution_id="exec_1"
            )
    ...
    def test_outer_exception_returns_fail_workflow(self):
        """An unexpected exception in the outer try returns fail_workflow (lines 1008-1010)."""
        with (
            patch("earth2studio.serve.server.cpu_worker.config") as mock_config,
            patch("earth2studio.serve.server.cpu_worker.redis_client") as mock_redis,
        ):
        ...
        assert result is not None
        assert result.get("success") is False
        assert "geocatalog" in result.get("error", "").lower()
